...
makeLeg(2);
    ...
    const partsMap = {
      ...
      function highlightPart(part) {
        ...
        const mesh = part.isMesh ? part : part.children[0];
        if (!mesh) return;

        const highlightGeom = mesh.geometry.clone();
        currentHighlight = new THREE.Mesh(highlightGeom, highlightMat);
        currentHighlight.scale.multiplyScalar(1.08);
        mesh.add(currentHighlight);
      }

document.querySelectorAll('.part-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    const partKey = btn.dataset.part;
    selectedPart = partsMap[partKey];

    if (selectedPart) {
    control.attach(selectedPart); 
...
rotateLeftBtn.addEventListener('mousedown', () => arrowRotateDir = -0.5);
rotateRightBtn.addEventListener('mousedown', () => arrowRotateDir = 0.5);
...
captureBtn.addEventListener('click', () => {
  ...
  const prevHighlightVisible = highlightMat.visible;
  const prevControlsVisible = control.visible;
  ring.visible = false;
  point.visible = false;
  highlightMat.visible = false;
  control.visible = false;
  ...
  highlightMat.visible = prevHighlightVisible;
  control.visible = prevControlsVisible;
  preview.innerHTML = '';
  const img = document.createElement('img');
  img.src = dataURL;
  preview.appendChild(img);
  const link = document.createElement('a');
  link.download = 'bloxd-thumbnails.png';
  link.href = dataURL;
